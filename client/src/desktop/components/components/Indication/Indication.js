...
class Indication extends Component {
  ...
  onActive = () => {
    ...
    this.tlOnActive.set(this.ref, {opacity: 1, transform: 'translateY(0)'}, "+=0")
    ...
    return (
      <div ref={this.initTimeline} className={classNames("indication",
        ...
        <div className="indication__title">
          ...
          {playerIndication.title &&
            <TextAnimation letterDuration={300} text={playerIndication.title} handleWord={word => this.word = word}/>
          }
        </div>
        ...
        <span className="indication__iconContainer">
          <svg className="indication__iconContainer__icon" viewBox="0 0 16 32">
            {playerIndication.theme === themes.white && <use xlinkHref="#icon-mobile"/>}
            {playerIndication.theme === themes.black && <use xlinkHref="#icon-mobile-2"/>}
          </svg>
        </span>
      </div>
    )
